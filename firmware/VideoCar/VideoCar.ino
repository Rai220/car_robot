...
// AP settings (channel / hidden / maxconnection) come from wifi_config.h
...
void setup() {
  ...
  if (err != ESP_OK) {  //ESP_OK
    // Don't abort: keep going so WiFi + motor control still come up without a camera.
    Serial.printf("Camera init failed with error 0x%x -- continuing without camera (WiFi + motors still work)\n", err);
  } else {
    //drop down frame size for higher initial frame rate
    sensor_t* s = esp_camera_sensor_get();
    s->set_framesize(s, FRAMESIZE_QVGA);
    //Video flip code — 0/0 = upright. Tune live: /control?var=vflip&val=0|1, var=hmirror&val=0|1
    s->set_vflip(s, 0);
    s->set_hmirror(s, 0);
  }
