...
int speed = 100;
int trim = 0;
int turnspeed = 60;  // in-place turn power (was hardcoded 150 = too fast; 60 chosen as gentle). Tune live: /control?var=turnspeed&val=N
...
static esp_err_t cmd_handler(httpd_req_t *req) {
  ...
  if (!strcmp(variable, "framesize")) {
    ...
    if (s && s->pixformat == PIXFORMAT_JPEG) res = s->set_framesize(s, (framesize_t)val);
  } else if (!strcmp(variable, "quality")) {
    Serial.println("quality");
    if (s) res = s->set_quality(s, val);
  } else if (!strcmp(variable, "vflip")) {
    Serial.println("vflip");
    if (s) res = s->set_vflip(s, val);
  } else if (!strcmp(variable, "hmirror")) {
    Serial.println("hmirror");
    if (s) res = s->set_hmirror(s, val);
  }
  //Remote Control
  ...
  } else if (!strcmp(variable, "trim"))  //trim
  {
    ...
    trim = val;
  } else if (!strcmp(variable, "turnspeed"))  //in-place turn power 0-255
  {
    if (val < 0) val = 0;
...
//      Serial.println("Backward");
    ...
    {
      i2c_Write(M1_PWM, turnspeed);
      i2c_Write(M1_IN, 0);
      i2c_Write(M2_PWM, turnspeed);
      i2c_Write(M2_IN, 0);
    } else if (val == 4)  //Right
    {
      Serial.println("右转");
      // Car_right();
      i2c_Write(M1_PWM, 0);
      i2c_Write(M1_IN, turnspeed);
      i2c_Write(M2_PWM, 0);
      i2c_Write(M2_IN, turnspeed);
    } 
     else if(val == 5) //Stop
    {
 //     Serial.println("Stop");
